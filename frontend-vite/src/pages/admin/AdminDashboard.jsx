...
const AdminDashboard = () => {
  ...
  const getUserActivityData = () => {
    ...
    activities.forEach(activity => {
      if (activity && activity.user_id !== undefined) {
        if (!userActivity[activity.user_id]) {
          userActivity[activity.user_id] = 0;
        }
        userActivity[activity.user_id] += activity.duration || 0;
      }
    });
    
    return Object.entries(userActivity).map(([userId, time]) => ({
      name: `User ${userId}`,
      time: Math.round((time / 3600) * 10) / 10, // Convert to hours with 1 decimal
    }));
  };
  ...
  const getAppUsageData = () => {
    ...
    activities.forEach(activity => {
      if (activity && activity.app) {
        ...
        appUsage[activity.app] += activity.duration || 0;
      }
    });
